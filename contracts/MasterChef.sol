...
// MasterChef is the master of BFARM. He can make BFARM and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once BFARM is sufficiently
...
contract MasterChef is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BFARMs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBFARMPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBFARMPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BFARMs to distribute per block.
        uint256 lastRewardTime; // Last block number that BFARMs distribution occurs.
        uint256 accBFARMPerShare; // Accumulated BFARMs per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        bool isNFTPool; // if lastRewardTime has passed
    }

    // The BFARM TOKEN!
    BFARMToken public BFARM;
    // The BFARM Address
    address public BFARMAddr;
    // Zap address
    address public zapAddr;
    // Dev address.
    address public devaddr;
    // BFARM tokens created per block.
    uint256 public BFARMPerSecond = 0;
    // total dev alloc
    uint256 public totalDevAlloc;
    // Bonus muliplier for early BFARM makers.
    ...
    // The timestamp when BFARM mining starts.
    ...
    // Update the given pool's BFARM allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function getMultiplier(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_fromTime <= startTime) return _toTime.sub(startTime).mul(BFARMPerSecond);
        return _toTime.sub(_fromTime).mul(BFARMPerSecond);
    }

    // View function to see pending BFARMs on frontend.
    function pendingBFARM(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accBFARMPerShare = pool.accBFARMPerShare;
        if (pool.isNFTPool) {
            uint256 lpSupply = IERC721(pool.lpToken).balanceOf(address(this));
            if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
                uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
                uint256 BFARMReward = multiplier.mul(pool.allocPoint).div(totalAllocPoint);
                accBFARMPerShare = accBFARMPerShare.add(
                    BFARMReward.mul(1e18).div(lpSupply.mul(amountPerNFT))
                );
            }
            return
                user.amount.mul(amountPerNFT).mul(accBFARMPerShare).div(1e18).sub(user.rewardDebt);
        } else {
            uint256 lpSupply = IERC20(pool.lpToken).balanceOf(address(this));
            if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
                uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
                uint256 BFARMReward = multiplier.mul(pool.allocPoint).div(totalAllocPoint);
                accBFARMPerShare = accBFARMPerShare.add(BFARMReward.mul(1e18).div(lpSupply));
            }
            return user.amount.mul(accBFARMPerShare).div(1e18).sub(user.rewardDebt);
        }
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (pool.isNFTPool) {
            ...
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 BFARMReward = multiplier.mul(pool.allocPoint).div(totalAllocPoint);
            BFARMToken(BFARM).mint(devaddr, BFARMReward.div(5));
            BFARMToken(BFARM).mint(address(this), BFARMReward);
            totalDevAlloc += BFARMReward.div(5);
            pool.accBFARMPerShare = pool.accBFARMPerShare.add(
                BFARMReward.mul(1e18).div(lpSupply.mul(amountPerNFT))
            );
            pool.lastRewardTime = block.timestamp;
        } else {
            ...
            BFARMToken(BFARM).mint(address(this), BFARMReward);
            totalDevAlloc += BFARMReward.div(5);
            pool.accBFARMPerShare = pool.accBFARMPerShare.add(BFARMReward.mul(1e18).div(lpSupply));
            pool.lastRewardTime = block.timestamp;
        }
    }

    // Deposit LP tokens to MasterChef for BFARM allocation.
    function deposit(uint256 _pid, uint256 _amount, bool isNFTAll) public nonReentrant {
        _deposit(_pid, _amount, isNFTAll);
    }

    /// @notice Deposit tokens to MasterChef for BFARM allocation.
    ...
    function depositFor(uint256 _pid, uint256 _amount, address _recipient) external nonReentrant {
        ...
        if (user.amount > 0) {
            if (pool.isNFTPool) {
                uint256 pending = user
                    .amount
                    .mul(amountPerNFT)
                    .mul(pool.accBFARMPerShare)
                    .div(1e18)
                    .sub(user.rewardDebt);
                if (pending > 0) {
                    safeBFARMTransfer(_recipient, pending);
                }
            } else {
                uint256 pending = user.amount.mul(pool.accBFARMPerShare).div(1e18).sub(
                    user.rewardDebt
                );
                if (pending > 0) {
                    safeBFARMTransfer(_recipient, pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accBFARMPerShare).div(1e18);
    ...
    function _deposit(
        ...
        if (user.amount > 0) {
            if (pool.isNFTPool) {
                uint256 pending = user
                    ...
                    .sub(user.rewardDebt);
                if (pending > 0) {
                    safeBFARMTransfer(msg.sender, pending);
                }
            } else {
                uint256 pending = user.amount.mul(pool.accBFARMPerShare).div(1e18).sub(
                    user.rewardDebt
        ...
        if (_amount > 0) {
            if (pool.isNFTPool) {
                uint256[] memory tokenIds = IBFARMNFT(pool.lpToken).walletOfOwner(_sender);
                if (isNFTAll) {
        ...
        if (pool.isNFTPool) {
            user.rewardDebt = user.amount.mul(amountPerNFT).mul(pool.accBFARMPerShare).div(1e18);
        } else {
            user.rewardDebt = user.amount.mul(pool.accBFARMPerShare).div(1e18);
        }
        emit Deposit(msg.sender, _pid, _amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount, bool isNFTAll) public nonReentrant {
        ...
        address _sender = msg.sender;
        if (pool.isNFTPool) {
            uint256 pending = user.amount.mul(amountPerNFT).mul(pool.accBFARMPerShare).div(1e18).sub(
                user.rewardDebt
            );
            if (pending > 0) {
                safeBFARMTransfer(msg.sender, pending);
            ...
            uint256 pending = user.amount.mul(pool.accBFARMPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeBFARMTransfer(msg.sender, pending);
            }
        }
    ...
    // Safe BFARM transfer function, just in case if rounding error causes pool to not have enough BFARMs.
    function safeBFARMTransfer(address _to, uint256 _amount) internal {
        uint256 BFARMBal = BFARM.balanceOf(address(this));
        if (_amount > BFARMBal) {
            BFARM.transfer(_to, BFARMBal);
        } else {
            BFARM.transfer(_to, _amount);
        }
    }
    ...
    function compound(uint256 _pid) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (user.amount > 0) {
                if (pending > 0) {
                    if (pending > BFARMBal) {
                        IERC20(BFARMAddr).safeIncreaseAllowance(address(zapAddr), BFARMBal);
                        amountOut = IZap(zapAddr).universalZapForCompound(
                            BFARMAddr, //_inputToken
                            BFARMBal, //_amount
                            pool.lpToken, //_targetToken
                            address(this) //_recipient
                        );
                    } else {
                        IERC20(BFARMAddr).safeIncreaseAllowance(address(zapAddr), pending);
                        amountOut = IZap(zapAddr).universalZapForCompound(
                            BFARMAddr, //_inputToken
                            pending, //_amount
                            pool.lpToken, //_targetToken
    ...
    function updateEmissionRate(uint256 _BFARMPerSecond) public onlyOwner {
        massUpdatePools();
        BFARMPerSecond = _BFARMPerSecond;
    }

    function setBFARMX(address _BFARM) public onlyOwner {
        require(_BFARM != address(0), "Invalid Address");
        BFARM = BFARMToken(_BFARM);
    }
