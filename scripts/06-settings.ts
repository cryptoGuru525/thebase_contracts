...
async function main() {
    ...
    const token = await ethers.getContractAt("BFARMToken", config.bfarm);
    const zapper = await ethers.getContractAt("ZapV3", config.zap);
    const nft = await ethers.getContractAt("BFARMNFT", config.nft);
    const masterchef = await ethers.getContractAt("MasterChef", config.masterchef);
    const presale = await ethers.getContractAt("BFARMPresale", config.presale);
